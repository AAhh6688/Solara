...
export async function onRequest(context) {
  ...
  try {
    const apiBase = MUSIC_SOURCES.qq[0];
    ...
    const nextAPI = getNextAPI('qq');
    console.log(`QQ音乐API失败,切换到备份: ${nextAPI}`);
    return new Response(JSON.stringify({ error: 'API失败' }), { status: 500 });
  }
}

async function searchSong(apiBase: string, keyword: string) {
  const response = await fetch(`${apiBase}/search?key=${encodeURIComponent(keyword)}`);
...
async function getSongUrl(apiBase: string, id: string) {
  const response = await fetch(`${apiBase}/song?songmid=${id}`);
  const data = await response.json();
  return new Response(JSON.stringify(data), {
    headers: { 'Content-Type': 'application/json' }
  });
...
async function getLyric(apiBase: string, id: string) {
  const response = await fetch(`${apiBase}/lyric?songmid=${id}`);
  const data = await response.json();
  return new Response(JSON.stringify(data), {
    headers: { 'Content-Type': 'application/json' }
  });
...
async function getPlaylist(apiBase: string, id: string) {
  const response = await fetch(`${apiBase}/songlist?id=${id}`);
  const data = await response.json();
  return new Response(JSON.stringify(data), {
    headers: { 'Content-Type': 'application/json' }
  });
